...
module.exports = function (bot) {
    return function (message, args) {
        if (!args[0] && !args[1]) {
            ...
            return bot.sendMessage(new bot.classes.Message(message.chat.id, 'Select the month to view expenses for.', null, null, null, null, keyboard), (res) => {});
        }

        var callback = function (err, all) {
            var sum = eu.sumUp(all);
            bot.sendMessage(new bot.classes.Message(message.chat.id, `You have spent ${sum} till now.`, null, null, null, null, new bot.classes.ReplyKeyboardHide), () => {});
        };
