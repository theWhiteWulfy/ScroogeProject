...
module.exports = function (bot) {
    return function (message, args) {
        if (!args[0] || !args[1] || _.isNaN(parseFloat(args[0]))) {
            return bot.sendMessage(new bot.classes.Message(message.chat.id, `Sorry, your command must look like this: \`${cfg.COMMANDS.NEW} 79.9 Dinner\`\nOptionally you could also specify a category using a hash tag like this:\`${cfg.COMMANDS.NEW} 79.99 Dinner #food\`. You can also simply leave out the \`${cfg.COMMANDS.NEW}\` and only type \`79.99 Dinner #food.\``, 'Markdown'), () => {
            });
        }
